import { BookOpen, FilePlus2, FileText, LayoutDashboard, LogOut, Menu, Moon, PackagePlus, Receipt, ShieldCheck, Sun, X } from "lucide-react";
...
const linkClass = ({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium ${isActive ? "bg-red-50 text-red-700 dark:bg-red-950/50 dark:text-red-300" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`;
...
export default function AppShell() {
  ...
  const brand = <div className="flex items-center gap-2 px-2 text-xl font-bold text-slate-900 dark:text-white"><span className="grid h-9 w-9 place-items-center rounded-xl bg-red-600 text-white"><BookOpen size={19} /></span>SEO</div>;
