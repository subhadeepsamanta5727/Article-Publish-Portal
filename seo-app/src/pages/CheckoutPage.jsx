...
export default function CheckoutPage() {
  ...
  return <div className="mx-auto max-w-3xl"><Link className="inline-flex items-center gap-2 text-sm font-semibold text-red-600" to="/articles/new"><ArrowLeft size={16} /> Back to package selection</Link><div className="card mt-5 p-6 sm:p-8"><PackageCheck className="text-red-600" size={30} /><p className="mt-5 text-sm font-semibold text-red-600">CHECKOUT</p><h1 className="mt-1 text-3xl font-bold">Review your publication order</h1><p className="mt-2 text-slate-500">One secure payment unlocks all {articleIds.length} article submissions.</p><div className="mt-7 rounded-xl bg-slate-50 p-5"><div className="flex justify-between text-sm"><span>Article quantity</span><span className="font-semibold">{articleIds.length}</span></div><div className="mt-3 flex justify-between border-t border-slate-200 pt-3 text-lg"><span className="font-bold">Total</span><span className="font-bold text-red-700">₹{total.toFixed(2)}</span></div><p className="mt-3 text-xs text-slate-500">The backend recalculates the final amount before Razorpay opens.</p></div><button disabled={busy} onClick={pay} className="btn-primary mt-6 w-full"><CreditCard size={18} />{busy ? "Preparing payment..." : `Pay ₹${total.toFixed(2)}`}</button></div></div>;
}
